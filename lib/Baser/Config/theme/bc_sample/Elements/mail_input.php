	...
	foreach ($mailFields as $key => $record) {
		...
		if ($field['use_field'] && ($blockStart && $iteration >= $blockStart) && (!$blockEnd || $iteration <= $blockEnd)) {
			...
			if ($group_field != $field['group_field'] || (!$group_field && !$field['group_field'])) {
				...
				if ($field['not_empty']) {
					echo '<span class="required">' . __('Required') . '</span>';
				} else {
					echo '<span class="normal">' . __('Any') . '</span>';
				}
				echo '</th>' . "\n" . '        <td class="col-input">';
			}
			...
			if (($this->BcArray->last($mailFields, $key)) ||
				...
				if ($field['group_valid']) {
					...
					echo $this->Mailform->error("MailMessage." . $field['group_field'] . "_not_same", __("Input data do not match."));
					echo $this->Mailform->error("MailMessage." . $field['group_field'] . "_not_complate", __("Input data is incomplete."));
				}
