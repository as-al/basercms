 ...
 */
	public function read($key) {
		...
		foreach (self::$variables as $var => $name) {
			if (!isset($$var)) {
				throw new ConfigureException(__d('baser_dev', '%s of the theme is not set : %s', array($name, $file)));
			}
			$config[$var] = $$var;
		}
		return $config;
 ...
 */
	protected function _getFilePath($key) {
		$dir = $this->_path . $key;
		if (!is_dir($dir)) {
			throw new ConfigureException(__d('baser_dev', 'A directory with the specified theme name does not exist. : %s', $dir));
		}
