...
<?php if ($blogContent['BlogContent']['comment_use']): ?>
	<div id="BlogComment">
		...
		<h4 class="contents-head"><?php echo __('Comment on this article') ?></h4>
		...
		<h4 class="contents-head"><?php echo __('Send comments') ?></h4>
		...
		<table cellpadding="0" cellspacing="0" class="row-table-01">
			<tr>
				<th><?php echo $this->BcForm->label('BlogComment.name', __('Name') . '・' . __('Nickname')) ?><span style="color:red">＊</span></th>
				<td><?php echo $this->BcForm->input('BlogComment.name', array('type' => 'text')) ?></td>
			</tr>
			<tr>
				<th><?php echo $this->BcForm->label('BlogComment.email', __('Email')) ?><span style="color:red">＊</span></th>
				<td>
					<?php echo $this->BcForm->input('BlogComment.email', array('type' => 'text', 'size' => 30)) ?>&nbsp;
					<small>※ <?php echo __('Email will not be published') ?></small>
				</td>
			</tr>
			<tr>
		...
		<?php if ($blogContent['BlogContent']['auth_captcha']): ?>
			<div class="auth-captcha clearfix">
				...
				&nbsp;<?php echo __('Please enter the letters of the image') ?><br />
			</div>
		<?php endif ?>

		<?php echo $this->BcForm->end(array('label' => __('Submit'), 'id' => 'BlogCommentAddButton', 'class' => 'button')) ?>
