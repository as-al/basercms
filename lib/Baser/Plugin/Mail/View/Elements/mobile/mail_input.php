			...
			<?php if (!$blockEnd || $iteration <= $blockEnd): ?>
				...
				<?php if (!$record['MailField']['group_valid']): ?>
					<?php if ($this->Mailform->error("MailMessage." . $record['MailField']['field_name'] . "_format", "check")): ?>
						<font color="#FF0000"><?php echo $this->Mailform->error("MailMessage." . $record['MailField']['field_name'] . "_format", __("Invalid format"), array('wrap' => false)); ?></font>
					<?php else: ?>
				...
				<?php if (($this->BcArray->last($mailFields, $key)) ||
					...
					<?php if ($record['MailField']['group_valid']): ?>
						<?php if ($this->Mailform->error("MailMessage." . $record['MailField']['group_field'] . "_format", "check")): ?>
							<font color="#FF0000"><?php echo $this->Mailform->error("MailMessage." . $record['MailField']['group_field'] . "_format", __("Invalid format"), array('wrap' => false)) ?></font>
						<?php else: ?>
							...
							<?php endif; ?>
						<?php endif; ?>
						<font color="#FF0000"><?php echo $this->Mailform->error("MailMessage." . $record['MailField']['group_field'] . "_not_same", __("Input data does not match"), array('wrap' => false)) ?></font>
						<font color="#FF0000"><?php echo $this->Mailform->error("MailMessage." . $record['MailField']['group_field'] . "_not_complate", __("Input data is incomplete"), array('wrap' => false)) ?></font>
					<?php endif; ?>
				<?php endif; ?>
